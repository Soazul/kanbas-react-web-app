...
export default function WorkingWithArraysAsynchronously() {
    ...
    const removeTodo = async (todo: any) => {
        const updatedTodos = await client.removeTodos(todo);
    ...
    const updatedTodo = async (todo: any) => {
        try {
            ...
            <ul className="list-group">
                {todos?.map((todo) => (
                    <li key={todo.id} className="list-group-item">
                        ...
                        <input type="checkbox" className="form-check-input me-2" defaultChecked={todo.completed} onChange={(e) => updatedTodo({...todo, completed: e.target.checked})}/>
                        {!todo.editing ? (todo.title) : (
                            <input className="form-control w-50 float-start" defaultValue={todo.title} 
                            onKeyDown={(e) => {
                                if (e.key === "Enter") {
                                    updatedTodo({...todo, editing: false});
                                }
                            }}
                            onChange={(e) => updatedTodo({...todo, title: e.target.value})}
                            />
                        )}
